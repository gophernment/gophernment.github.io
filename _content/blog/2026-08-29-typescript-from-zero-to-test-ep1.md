...
## เตรียม Node version ไว้ด้วย (fnm)

แม้ซีรีส์นี้จะใช้ Bun เป็นหลัก แต่ยังมีหลายจุดที่ต้องใช้ Node.js จริงอยู่ เช่น Next.js ในตอนที่ 7, ตัว CI ที่รัน build, หรือ library บางตัวที่พึ่ง Node ล้วน ๆ

best practice ของคนเขียน JS/TS คือ**แยกการจัดการ Node version ออกจากตัวระบบ** ด้วยเครื่องมือชื่อ **fnm** (Fast Node Manager) — เร็ว และสลับเวอร์ชันตามโปรเจกต์ได้อัตโนมัติ

```bash
# ติดตั้ง fnm
curl -fsSL https://fnm.vercel.app/install | bash

# ติดตั้ง Node LTS + ตั้งเป็นค่า default
fnm install --lts
fnm default lts-latest
```

ติดตั้งเสร็จเปิด terminal ใหม่ แล้วเช็ค

```bash
node --version
```

จุดเด่นของ fnm คือการ**สลับเวอร์ชันอัตโนมัติ** — พอเข้าโฟลเดอร์ที่มีไฟล์ `.node-version` ระบุเวอร์ชันไว้ fnm จะ switch ให้เองทันที ไม่ต้องรันคำสั่งเปลี่ยนเวอร์ชันเองทุกครั้ง (ตอนนี้เรายังไม่ต้องใช้ Node โดยตรง แต่พอถึงตอนที่ 7 ซึ่งต้องรัน Next.js คุณจะได้ใช้จริง)
